...
```kts
compileOnly(libs.lombok) // Версионированные каталоги
...
2. **`@ToString`**: генерация toString метода, возвращающего информацию об объекте в виде строки
	- Параметр `includeFieldNames=true` перед каждым полем в строку добавляет его название
3. **`@EqualsAndHashCode`**: методы equals (логическое равенство объектов) и hashCode (хеш-код объекта) 
	- Аннотации `@EqualsAndHashCode.Include` и `.Exclude` на полях позволяют исключить/добавить использование поля в методах
	- Параметр `onlyExplicitlyIncluded = true` включает только поля, помеченные `.Include`
...
6. **`@NonNull`**: генерирует для поля команды для проверки на ноль в конструкторе
7. **`@With`**: позволяет создать методы `withField`, позволяющие создать копию объекта с одним измененным полем
...
---
## 5. Особенности использования в [[Spring - Index|Spring Boot]]

1. Использование `@Data` и `@EqualsAndHashCode` нежелательно в чистом виде на JPA Entity из за возможных бесконечных рекурсий при связах сущностей (напр. `@ManyToOne`). 
	- В `@EqualsAndHashCode` включать только уникальные поля сущности или прописывать методы вручную
...
2. **`@Cleanup`**: генерирует код очистки заданного ресурса (почти не используется, try-with-resources предпочтительнее)